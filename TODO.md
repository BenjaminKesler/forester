...
1. **Enemy fade-in to 5s.** Change the creature spawn-in delay from 1.0s to 5.0s
   — `CREATURES.spawn.emergeSec` in `js/config.js`. (The emerge mechanic already
   exists: creatures are attackable but inert while emerging.)

2. **Death message.** When the player dies, show a message that says they died,
...
3. **Death resets spawning to a fresh state.** After a death, enemies should stop
...
4. **Value-per-Heartwood indicators.** Below each shop item's price, show its
...
5. **Sound effects.** Add audio (clicks, kills, fells, death). Note: CLAUDE.md
